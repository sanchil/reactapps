     ...
     // console.log('google token: local storage: '+ localStorage.getItem('googleToken'));
      ...
      if (agtSessData['access_token']) {
        ...
        getEmailUserSvr(agtSessData['access_token'])
          .then(res => {
            qry.addSocialUser('google',res, dispatch);

            //   console.log("ID is : "+ JSON.stringify(res));
            //console.log('Google user: ' + JSON.stringify(res));
         /*    setCookie('sessid', 'SESSION_GOOGLE_' + res.id);
            sessionStorage.setItem('sessid', 'SESSION_GOOGLE_' + res.id);
            setRedis('SESSION_GOOGLE_' + res.id, res);
            dispatch({ type: 'LOGIN', login: true, sessid: 'SESSION_GOOGLE_' + res.id, source: 'google', sessdata: {}, data: res });
 */
    ...
    if (
     ...
     && location.hash !== ""
      ...
      //getFBUser('EAAIojKWrSbABAIpoo85H2ZCKFmvDvsrf2ZCfYDQofHPUbzc8oKZAkGdZA2oEq6j22ZASCNIfqWHPsP9qQU4sYPRZCjbY01sThsQM3yI4lOQAryXKdNfsLZCWGnCBdMRe2lIiDHxaEaTZC0Eib12tht8dVA6QqOa5oI01XRDgsc0FqinGzYanJom2')
       ...
       getFBUser()
        .then(res => {
          qry.addSocialUser('facebook',res, dispatch);

     /*      console.log('FB user: ' + JSON.stringify(res));
          setCookie('sessid', 'SESSION_FB_' + res.id);
          sessionStorage.setItem('sessid', 'SESSION_FB_' + res.id);
    ...
    */
    ...
    if (
      location.pathname === '/amzauth'
   //   && location.hash
  //    && (typeof location.hash !== 'undefined')
    //  && location.hash !== ""
      ) {

        qry.addSocialUser('amazon',agtSessData, dispatch);

     /*    console.log("Amazon user is "+ JSON.stringify(agtSessData));
        setCookie('sessid', 'SESSION_AMZ_' + agtSessData.user_id);
        sessionStorage.setItem('sessid', 'SESSION_AMZ_' + agtSessData.user_id);
        setRedis('SESSION_AMZ_' + agtSessData.user_id, agtSessData);
        dispatch({ type: 'LOGIN', login: true, sessid: 'SESSION_AMZ_' + agtSessData.user_id, source: 'amazon', sessdata: {}, data: agtSessData }); */
  ...
  /*     getAmzUser()
    ...
    if (location.pathname === '/twauth') {
      ...
      if (sessObj) {
        ...
        qry.addSocialUser('twitter',sessObj, dispatch);

      /*   setCookie('sessid', 'SESSION_TW_' + sessObj.user_id);
        sessionStorage.setItem('sessid', 'SESSION_TW_' + sessObj.user_id);
